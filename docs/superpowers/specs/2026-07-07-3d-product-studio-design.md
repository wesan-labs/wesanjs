...
- **Faz A — üretim hattı (turntable).** Yükleme UI + **Flux2→Seedance→SeeDVR pipeline** (fal/replicate) → 72-kare 4K orbital turntable + asset store + **kare-seçici viewer** (hero kareyi seç/yakala).
  ...
  **Somut dilimler (build-yeşil sırası — Tripo yolu retire edilene kadar yanında yaşar):**
  ...
  - [x] **A2 · ① BFL Flux2 hero adaptörü** — `providers/bfl.ts`: DOĞRULANMIŞ `/v1/flux-2-pro` sözleşmesi (submit→`polling_url`→`result.sample`, header `x-key`). `mapBflStatus`/`buildHeroPrompt`/`toFluxBody` saf/test'li (8-referans sınırı, hex→prompt, `input_image_N` düz alan). `pipeline.ts`: `StepJob.pollUrl` eklendi + `poll(job)` sözleşmesi düzeltildi (BFL polling_url zorunlu). `BFL_API_KEY` env (helm/.env). **CANLI-DOĞRULANDI (2026-07-08):** auth+endpoint+gövde çalışıyor; `input_image` URL kabul (base64 değil); tek engel BFL kredi (`Insufficient credits`). **Kısıt:** kaynak URL BFL'in DOĞRUDAN çekebileceği olmalı (redirect/403 → "Unable to extract dimensions") → A4 re-host. *(2026-07-08)*
  - [x] **A2b · ②③ video adaptörleri** — `byteplus.ts` (Seedance orbital, Ark `content[]` first_frame+reference_image, `Bearer`, poll→`content.video_url`) + `wavespeed.ts` (SeedVR upscale, `{video, target_resolution:"4k"}`, poll→`data.outputs[0]`). Doğrulanmış sözleşmeler (doküman; canlı-test key gelince). `mapArkStatus`/`mapWaveSpeedStatus`/`toSeedanceBody`/`toUpscaleBody` saf/test'li. env: `ARK_API_KEY` + `WAVESPEED_API_KEY`. Tam zincir onaylı. *(2026-07-08)*
  - [x] **A3 · veri modeli** — `product-3d-asset` additive alanlar: `pipeline_step`+`step_job_id`+`step_poll_url`+`hero_url`+`video_url`+`turntable_urls[]` (eski Tripo kolonları korundu). Migration UYGULANDI (helm DB'de kolonlar doğrulandı). *(2026-07-08)*
  - [x] **A4 · zincir workflow** — `create/poll` → **poll-on-read state-machine** (`step-registry.ts` adım→adaptör+env eşleme): her poll aktif adımın job'ı yoksa submit, varsa poll eder; hazırsa çıktıyı saklar + `pipeline_step` ilerletir (hero→orbital→upscale→done). Key eksikse `processing` kalır. tsc temiz. *(2026-07-08)* `sample` adımında SeeDVR mp4'ünü `sampleTimestamps`'te kare-çıkar → `turntable_urls`. Terminal: `done`. **Re-host zorunlu (canlı-kanıt):** (a) kullanıcı yüklemesi → S3/R2 direkt URL (BFL redirect/403 izlemez); (b) her adım çıktısı 10 dk'da expire → indir+sakla.
  - [x] **A5 · UI** — `three-d-demo.tsx` (`/content/3d-demo`): pipeline stepper (① hero → ②③ video → done) + hero görsel + 4K orbital `<video>`. GLB→capture akışı çıktı (Faz B'ye). *(2026-07-08)*
  ...
  - [x] **Re-host** — her adım çıktısı Medusa file service'e indirilir (`rehost.ts`; düşerse remote'a fallback); zincir girdisi REMOTE URL ile AYNI poll tick'inde submit (expire penceresi kapalı). S3/R2'ye geçiş = file provider konfigürasyonu. *(2026-07-09)*
  - [x] **Ürün-bazlı kütüphane** — `product_ref` (asset+content_item, migration UYGULANDI); 3D sekmesinde ürün adı/kodu alanı; hero (image) + final 4K video (video) kütüphaneye otomatik `product_ref` ile kaydolur. KALAN: LibraryDrawer'da görsel gruplama UI + video kind desteği. *(2026-07-09)*
  - [ ] **Faz B / GLB adayı (keşif):** BytePlus konsolunda Hyper3D-Gen2 (Rodin, 150K bedava token) + Hitem3D (500K) görüldü — kare→GLB için ARK key ile yeni-hesapsız yol.

  ▎ **v1 çıktı:** foto URL → hero → 360° 4K video (turntable frame'siz).
  ▎ **UÇTAN UCA CANLI (2026-07-08): TAM ZİNCİR ÇALIŞTI.** foto → ① Flux2 hero (~12sn) → ② Seedance `seedance-1-5-pro-251215` 360° orbital (2M bedava kota) → ③ SeedVR 4K (~130sn) = gerçek 4K dönen ürün videosu. Canlı-kanıtlar: (a) BFL `input_image` **data-URL de kabul ediyor** → stüdyö tuvalinden hero RE-HOST'SUZ çalışır; (b) Seedance 1.5: `first_frame` + `reference_image` **karıştırılamaz** → orbital'e ref gönderilmiyor (kimliği hero taşır); 2.0 Reference Cluster'da yeniden ele al; (c) çıktı URL'leri geçici (BFL ~10dk, Ark 24s) → kalıcılık için re-host dilimi hâlâ gerekli (akış için değil, SAKLAMA için). Dreamina-2.0'a geçiş = konsolda Activate + descriptor'da model değişikliği (tek satır).
