...
// PATCH /admin/tenants/:id/members/:membershipId — üyelik rolü / modül RBAC güncelle.
export const PATCH = async (
  ...
  const update: Record<string, unknown> = { id: membershipId }
  if (body.role !== undefined) {
    update.role = body.role
  }
  if (body.rbac_role_id !== undefined) {
    update.rbac_role_id = body.rbac_role_id
  }

  const updated = await service.updateTenantMemberships(update as any)
